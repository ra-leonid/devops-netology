...
yc resource-manager folder add-access-binding new --role editor --subject serviceAccount:ajeg5cggin4neosiios5
...
```
5. Выполняем первую инициализацию:
```commandline
...
# Важное примечание. Замените «worker-node01» на имя хоста любого из рабочих узлов вашего кластера.
kubectl apply -f volume.yaml

# Файл развертывания использует постоянный том класса локального хранилища для данных Jenkins. Для производственных сценариев использования вам следует добавить постоянный том облачного класса хранения для ваших данных Jenkins.
# Если вам не нужен постоянный том локального хранилища, вы можете заменить определение тома в развертывании каталогом хоста, как показано ниже.
...
kubectl logs jenkins-b96f7764f-kpjtz --all-containers

kubectl apply -f service.yaml
minikube ip
http://192.168.59.136:30002/

# запустить команду exec, чтобы получить пароль непосредственно из места:
kubectl exec -it jenkins-b96f7764f-kpjtz cat /var/jenkins_home/secrets/initialAdminPassword
...
kubectl apply -f jenkins-volume1.yaml
kubectl apply -f jenkins-sa.yaml
helm upgrade --install jenkins -n jenkins -f jenkins-values.yaml jenkins/jenkins
helm upgrade --install jenkins -n stage -f jenkins-values.yaml jenkins/jenkins
...
echo http://$(minikube ip):$(kubectl get -n jenkins -o jsonpath="{.spec.ports[0].nodePort}" services jenkins)/login

helm uninstall jenkins
kubectl delete -f jenkins-volume.yaml
kubectl delete -f jenkins-volume1.yaml
...
#helm repo update
#helm install nfs tranhailong/nfs-server --create-namespace -n nfs
#helm delete nfs -n nfs

helm install nfs-server stable/nfs-server-provisioner --create-namespace -n nfs




ssh -o "StrictHostKeyChecking=no" -o "UserKnownHostsFile=/dev/null" -o "ProxyCommand=\"ssh -W %h:%p ubuntu@51.250.80.228 -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\"" ubuntu@10.127.0.28

ANSIBLE_FORCE_COLOR=1 ansible-playbook -i inventory.yml nfs.yml

sudo mount -t nfs 10.127.0.28:/share /data/
```
...
kubectl -n ingress-nginx port-forward ingress-nginx-controller-xltg7 8080:80

curl -I meow-app.duckdns.org
```